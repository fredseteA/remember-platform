...
// ── Gera QR Code usando qrcodejs via CDN ──────────────────────────────────────
...
function StatPill({ label, value, accent }) {
  return (
    <div style={{
      ...
      <p style={{ fontSize: '0.73rem', color: '#7a8aaa', margin: '6px 0 0', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600 }}>
        {label}
      </p>
    </div>
  );
  ...
  return (
    <button
      ...
      style={{
        ...
        border: copied ? '1px solid #86efac' : '1px solid #e8edf4',
        borderRadius: 10,
        cursor: 'pointer',
        color: copied ? '#16a34a' : '#5a6a8a',
        fontSize: '0.83rem',
        fontFamily: '"Georgia", serif',
...
export function ApoiadorMeuCodigo() {
  ...
  const totalRevenue = sales
    .filter(s => !['cancelled', 'pending'].includes(s.status))
    .reduce((a, s) => a + (s.final_amount || s.amount || 0), 0);
  ...
  const handleShare = async () => {
    if (navigator.share) {
      await navigator.share({
        title: 'Remember QRCode',
        text: `Use meu código ${code} e ganhe desconto em memórias eternas ❤️`,
        url: referralLink,
      }).catch(() => {});
    } else {
  ...
  return (
    <ApoiadorLayout>
      <div style={{ maxWidth: 780, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ color: '#5a6a8a', fontSize: '0.86rem', margin: 0 }}>
              Compartilhe seu código e ganhe comissão em cada venda.
            </p>
          </div>
        ...
        <div style={{
          background: 'linear-gradient(135deg, #0f1f3d 0%, #1a3461 100%)',
          borderRadius: 20,
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{
              flex: 1, fontSize: '0.82rem', color: '#5a6a8a',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              {!qrReady && (
                <div style={{
                  ...
                  <RefreshCw size={20} style={{ color: '#9aaac0', animation: 'spin 1s linear infinite' }} />
                </div>
              )}
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              <p style={{ color: '#5a6a8a', fontSize: '0.85rem', lineHeight: 1.6, marginBottom: 20 }}>
                Este QR Code leva diretamente ao site com seu código de apoiador já aplicado. 
                Ideal para cartões, posts, panfletos e stories.
              </p>

              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <button
                  onClick={() => downloadQR(qrRef, `qrcode-apoiador-${code}.png`)}
                  disabled={!qrReady}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 8,
                    padding: '11px 18px',
                    background: qrReady ? '#1a2744' : '#e8edf4',
                    color: qrReady ? '#fff' : '#9aaac0',
                    border: 'none', borderRadius: 10,
                  ...
                  onMouseEnter={e => { if (qrReady) e.currentTarget.style.opacity = '0.85'; }}
                  onMouseLeave={e => e.currentTarget.style.opacity = '1'}
                >
                  <Download size={14} />
                  Baixar QR Code (PNG)
              ...
              <div style={{
                ...
                <p style={{ fontSize: '0.78rem', color: '#3b82f6', margin: 0, lineHeight: 1.5 }}>
                  💡 <strong>Dica:</strong> Compartilhe nas redes sociais, WhatsApp ou imprima para usar offline. Cada scan conta como uso do seu código!
                </p>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            {[
              ...
              <div key={label} style={{
                ...
                <p style={{ fontFamily: '"Georgia", serif', fontSize: '1.7rem', fontWeight: 700, color, margin: 0 }}>
                  {value}
                </p>
                <p style={{ fontSize: '0.73rem', color: '#7a8aaa', margin: '5px 0 0', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                  {label}
                </p>
              </div>
            ))}
