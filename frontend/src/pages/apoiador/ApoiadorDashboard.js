...
function getLevel(sales) {
  return LEVELS.find(l => sales >= l.min && sales <= l.max) || LEVELS[0];
}
function getNextLevel(sales) {
  const idx = LEVELS.findIndex(l => sales >= l.min && sales <= l.max);
...
function ProgressCard({ sales, level, nextLevel, delay = 0 }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      if (nextLevel) {
        const progress = ((sales - level.min) / (nextLevel.min - level.min)) * 100;
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          <p style={{
            fontSize: '0.73rem', color: '#7a8aaa',
            letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 6,
          }}>Progresso de Nível</p>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '0.78rem', color: '#5a6a8a' }}>
          {nextLevel ? (
            <><strong style={{ color: '#1a2744' }}>{sales}</strong> / <strong style={{ color: '#1a2744' }}>{nextLevel.min}</strong> vendas este mês</>
          ) : (
            <strong style={{ color: level.color }}>Nível máximo atingido! 🏆</strong>
          )}
        </span>
        {nextLevel && (
          <span style={{ fontSize: '0.75rem', color: '#9aaac0' }}>
            Faltam <strong style={{ color: nextLevel.color }}>{salesNeeded}</strong> para {nextLevel.rate}%
          </span>
        )}
      </div>
    </div>
...
function RecentSales({ sales }) {
  ...
  const STATUS_COLORS = {
    approved: '#16a34a', paid: '#16a34a', in_production: '#8b5cf6',
    produced: '#3b82f6', shipped: '#f59e0b', entregue: '#16a34a',
    cancelled: '#ef4444', pending: '#9aaac0',
  };
  const STATUS_LABELS = {
    approved: 'Aprovado', paid: 'Pago', in_production: 'Produção',
    produced: 'Produzido', shipped: 'Enviado', entregue: 'Entregue',
    cancelled: 'Cancelado', pending: 'Pendente',
  };

  if (!sales.length) return (
    <div style={{ textAlign: 'center', padding: '40px 0', color: '#9aaac0', fontSize: '0.88rem' }}>
      Nenhuma venda ainda. Compartilhe seu código! 🚀
    </div>
  );
        ...
        <thead>
          <tr style={{ borderBottom: '2px solid #f0f4f8' }}>
            {['Plano', 'Valor', 'Comissão', 'Status', 'Data'].map(h => (
              <th key={h} style={{
                padding: '10px 14px', textAlign: 'left', color: '#7a8aaa',
          ...
          {sales.slice(0, 5).map((s, i) => {
            ...
            return (
              <tr key={i} style={{ borderBottom: '1px solid #f0f4f8' }}>
                <td style={{ padding: '12px 14px', color: '#1a2744', fontWeight: 500 }}>
                  {PLAN_LABELS[s.plan_type] || s.plan_type}
                </td>
                <td style={{ padding: '12px 14px', color: '#1a2744' }}>
                  R$ {Number(s.final_amount || s.amount || 0).toFixed(2)}
                </td>
                <td style={{ padding: '12px 14px', color: '#16a34a', fontWeight: 600 }}>
                  R$ {Number(s.commission_amount || 0).toFixed(2)}
                </td>
                <td style={{ padding: '12px 14px' }}>
                  <span style={{
                    background: `${STATUS_COLORS[status] || '#9aaac0'}18`,
                    color: STATUS_COLORS[status] || '#9aaac0',
                    padding: '3px 10px', borderRadius: 20,
                    fontSize: '0.72rem', fontWeight: 600,
                  }}>{STATUS_LABELS[status] || status}</span>
                </td>
                <td style={{ padding: '12px 14px', color: '#7a8aaa', whiteSpace: 'nowrap' }}>{date}</td>
              </tr>
            );
          })}
        </tbody>
...
export default function ApoiadorDashboard() {
  ...
  const availableCommission = sales.reduce((acc, s) =>
    acc + (s.commission_status === 'available' ? (s.commission_amount || 0) : 0), 0
  );
    ...
    <ApoiadorLayout>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
              ...
              <h1 style={{
                ...
                fontWeight: 700, color: '#1a2744', margin: 0,
              }}>
        ...
        <div style={{
          ...
          <StatCard
            icon={TrendingUp} label="Vendido no mês"
            value={`R$ ${monthRevenue.toFixed(2)}`}
            sub={`${monthSalesCount} venda${monthSalesCount !== 1 ? 's' : ''} este mês`}
            accent="#16a34a" delay={0}
          />
          <StatCard
            icon={Coins} label="Comissão do mês"
            value={`R$ ${monthCommission.toFixed(2)}`}
            sub={`${level.rate}% aplicado este mês`}
            accent="#f59e0b" delay={80}
          />
          <StatCard
            icon={Wallet} label="Comissão disponível"
            value={`R$ ${availableCommission.toFixed(2)}`}
            sub="Pronta para recebimento"
            accent="#8b5cf6" delay={160}
          />
          <StatCard
            icon={Percent} label="Percentual atual"
            value={`${level.rate}%`}
            sub={`Nível: ${level.label}`}
            accent={level.color} delay={240}
          />
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div>
              ...
              <p style={{ fontSize: '0.75rem', color: '#9aaac0', margin: '3px 0 0' }}>
                Últimas 5 vendas com seu código
              </p>
            </div>
