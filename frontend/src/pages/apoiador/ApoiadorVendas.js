...
function SummaryCard({ icon: Icon, label, value, accent, delay = 0 }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: '0 2px 10px rgba(26,39,68,0.04)',
      display: 'flex',
      alignItems: 'center',
      gap: 14,
      opacity: visible ? 1 : 0,
...
// Máscara parcial do email do comprador
...
export function ApoiadorVendas() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(`${API}/apoiador/sales`, {
  ...
  const filtered = useMemo(() => {
    return sales.filter(s => {
      ...
      if (search) {
        ...
        const plan = (PLAN_LABELS[s.plan_type] || s.plan_type || '').toLowerCase();
        if (!email.includes(q) && !plan.includes(q)) return false;
      }
      return true;
    });
  ...
  return (
    <ApoiadorLayout>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <p style={{ color: '#5a6a8a', fontSize: '0.86rem', margin: 0 }}>
              {sales.length} venda{sales.length !== 1 ? 's' : ''} no total com seu código
            </p>
          </div>
        ...
        {/* Filtros */}
        <div style={{
          background: '#fff', borderRadius: 14, border: '1px solid #e8edf4',
          ...
          <div style={{
            ...
            <div style={{ position: 'relative', flex: 1, minWidth: 180 }}>
              ...
              <input
                ...
                placeholder="Buscar por plano ou email..."
                style={{
                  width: '100%', padding: '8px 10px 8px 30px',
                  border: '1px solid #e8edf4', borderRadius: 8,
                  fontSize: '0.83rem', color: '#1a2744',
            ...
            <button
              ...
              style={{
                ...
                padding: '8px 14px', border: '1px solid #e8edf4',
                borderRadius: 8, background: showFilters ? '#f0f4f8' : '#fff',
                cursor: 'pointer', color: '#5a6a8a', fontSize: '0.82rem',
            ...
            {activeFilters > 0 && (
              <button onClick={clearFilters} style={{
                ...
                borderRadius: 8, background: '#fff5f5',
          ...
          {showFilters && (
            <div style={{
              display: 'flex', gap: 12, padding: '0 16px 14px',
              flexWrap: 'wrap', borderTop: '1px solid #f0f4f8',
              paddingTop: 14,
            }}>
              {/* Mês */}
              <select
          ...
          {filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: '#9aaac0' }}>
              ...
              <p style={{ fontSize: '0.9rem', margin: 0 }}>
                {sales.length === 0 ? 'Nenhuma venda encontrada. Compartilhe seu código!' : 'Nenhuma venda com esses filtros.'}
              </p>
            </div>
          ) : (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.84rem' }}>
                ...
                <tbody>
                  {filtered.map((s, i) => {
                    ...
                    const pct = s.commission_rate
                      ? `${(s.commission_rate * 100).toFixed(0)}%`
                      : (valor > 0 ? `${((comissao / valor) * 100).toFixed(0)}%` : '—');
                    ...
                    return (
                      ...
                      >
                        <td style={{ padding: '13px 16px', color: '#5a6a8a', fontSize: '0.8rem' }}>
                          {maskEmail(s.user_email)}
                        </td>
                        <td style={{ padding: '13px 16px', color: '#1a2744', fontWeight: 500 }}>
                          {PLAN_LABELS[s.plan_type] || s.plan_type || '—'}
                        </td>
                        <td style={{ padding: '13px 16px', color: '#1a2744', fontWeight: 600 }}>
                          R$ {valor.toFixed(2)}
                        </td>
                        <td style={{ padding: '13px 16px' }}>
                          <span style={{
                            background: '#5aa8e018', color: '#5aa8e0',
                        ...
                        <td style={{ padding: '13px 16px', color: '#7a8aaa', whiteSpace: 'nowrap', fontSize: '0.8rem' }}>
                          {date}
                        </td>
                      </tr>
                    );
          ...
          {filtered.length > 0 && (
            <div style={{
              ...
              <span style={{ fontSize: '0.78rem', color: '#9aaac0' }}>
                Exibindo <strong style={{ color: '#1a2744' }}>{filtered.length}</strong> de <strong style={{ color: '#1a2744' }}>{sales.length}</strong> venda{sales.length !== 1 ? 's' : ''}
              </span>
              <span style={{ fontSize: '0.78rem', color: '#9aaac0' }}>
                Total de comissões: <strong style={{ color: '#16a34a' }}>R$ {totalCommission.toFixed(2)}</strong>
              </span>
            </div>
          )}
        </div>
      ...
      <style>{`
        ...
        select:focus { outline: none; border-color: #5aa8e0 !important; }
        input:focus { border-color: #5aa8e0 !important; }
      `}</style>
    </ApoiadorLayout>
  );
}
